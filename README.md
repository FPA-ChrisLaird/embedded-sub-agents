...
All agents should explicitly deny firmware flashing, programming, device I/O, and destructive build targets. Read-only subagents may generate reports, design options, and code proposals in their responses, but cannot modify the workspace or execute local commands. They may run only allow-listed, read-only `gh` commands to inspect repository context, issues, pull-request status and diffs, and GitHub Actions status and logs.
...
| GitHub CLI read access    | Allow listed `gh` read commands and read-only `gh api` queries; keep all other `gh` invocations at `ask` for the primary and denied for subagents.                                                     | Avoids approval prompts for repository context, issues, PR status/diffs, and Actions logs without granting mutation rights. |
