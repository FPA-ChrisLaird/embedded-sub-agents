...
| Repository inspection     | Allow listed `gh` read commands, read-only `gh api` queries, and exact non-mutating Git inspection commands; keep all other `gh` and Git invocations at `ask` for the primary and denied for subagents. | Avoids approval prompts for repository, PR, and Actions context plus local state without granting mutation rights. |
