...
- Allow the primary and analysis subagents to run only allow-listed, read-only GitHub CLI commands for repository context, issues, pull-request status and diffs, and GitHub Actions status and logs.
...
- Subagent file edits or local shell command execution. Read-only GitHub CLI retrieval is in scope.
...
- Read-only agents may use workspace read, glob, grep, and list operations, plus an allow-listed set of read-only `gh` commands applied by the shared plugin. They must deny edit, task, and all other shell permissions.
...
- The three analysis subagents are read-only and cannot edit files, execute local shell commands, or create child subagents. They may use only allow-listed, read-only GitHub CLI commands.
...
- Confirm each subagent returns the required hand-off sections, can use the allow-listed read-only GitHub CLI commands, and cannot call other prohibited tools.
