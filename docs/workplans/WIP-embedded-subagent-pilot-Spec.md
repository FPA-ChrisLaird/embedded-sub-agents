...
- Subagent file edits or shell command execution.
...
- Enabling Jira, Confluence, Xray, GitHub, or other external retrieval integrations for subagents during the initial pilot.
...
- The secret-free agent definitions and depth configuration are version-controlled in this repository as the canonical source, then installed into global OpenCode configuration for reuse across projects.
- Read-only agents may use workspace read, glob, grep, and list operations only. They must deny edit, bash, and task permissions.
...
- The three analysis subagents are read-only and cannot edit files, execute shell commands, or create child subagents.
...
- Confirm each subagent returns the required hand-off sections and cannot call tools prohibited by its permissions.
